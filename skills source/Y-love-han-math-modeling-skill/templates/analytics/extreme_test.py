    ...
    for name, (lo, hi, typical) in param_ranges.items():
        ...
        for val in test_values:
            ...
            try:
                ...
                issue = None
                # 检查输出合理性：非实数输出（None/复数）先归为 FAIL 而不是
                # 在后续数值比较中抛 TypeError 被误分类为 CRASH
                if output is None or isinstance(output, complex):
                    issue = f"输出非实数 (参数 {name}={val:.3g})"
...
def plot_boundary_test_results(boundary_results):
    ...
    # ⚠️ 图内文字禁用 emoji（SimHei 无对应字形，渲染为方块）——
    #    用纯文本状态标记
...
def generate_extreme_test_report(boundary, degenerate, noise, alternative):
    ...
    # 噪声
    lines.append("\n【3. 噪声鲁棒性测试】")
    for level, noise_stat in noise.items():
        flag = '✅' if noise_stat['degradation_pct'] < 10 else '❌'
        lines.append(f"  {flag} {level}: 退化 {noise_stat['degradation_pct']:.2f}%, "
                    f"95%CI: [{noise_stat['ci_95'][0]:.4f}, {noise_stat['ci_95'][1]:.4f}]")

    # 备选方案
    lines.append("\n【4. 备选方案对比】")
    if alternative:
        for name, alt_stat in alternative.items():
            diff = alt_stat['vs_primary_pct']
            direction = '优于' if diff > 0 else '劣于'
            lines.append(f"  {name}: 均值={alt_stat['mean']:.4f}, "
...
if __name__ == '__main__':
    ...
    def example_model(params):
        ...
        ⚠️ 示例对非法输入做 clamp（a<0 → 0），使示例默认全部
        PASS、退出码 0（演示"正常通过"形态）。真实模型中：
        - 若返回 NaN/Inf → 被标记 FAIL（边界输出检测）
        - 若抛异常     → 被标记 CRASH（崩溃检测）
        两者都是正常检测能力，示例只是演示"通过"形态。
        """
        a, b, c = params['a'], params['b'], params['c']
        if a < 0:
            a = 0.0  # 非法输入 clamp（示例行为；真实模型可按需改为 NaN/抛异常）
    ...
    noise_results = noise_robustness_test(example_model,
                                           {'a': 1.0, 'b': 1.0, 'c': 0.5})
    for level, noise_stat in noise_results.items():
        print(f"  {level}: 退化 {noise_stat['degradation_pct']:.2f}%")

    print("\n[4/4] 备选方案对比...")
    # 示例备选方案（演示"主方法最优"的合理情形——简化/近似版本性能
    # 均略逊于完整模型；若真实赛题中备选方案反而更优，须重新审视
    # 模型选择，这正是本测试的意义）
    alt_funcs = {
        '简化模型(无exp项)': lambda p: 0.9 * (p['a'] * np.sin(p['b']) + 1.0),
        '线性近似': lambda p: 0.85 * (p['a'] * p['b'] + 1.0 - p['c'] * p['a']),
    }
    test_inputs = [
        {'a': 0.5, 'b': 0.5, 'c': 1.0},
        {'a': 1.0, 'b': 1.0, 'c': 0.5},
    ...
    for name, alt_stat in alt_results.items():
        print(f"  {name}: vs 主方法 {alt_stat['vs_primary_pct']:+.1f}%")
    ...
    # ⛔ 正常结束标记（阶段 6.5 门禁：日志须含 ✅/完成/SUCCESS，否则
    #    判"脚本未实际运行"）——即使存在预期失败项（如负参数 CRASH），
    #    也先打印结束标记再退出，门禁据此确认脚本已真实执行
    print("\n✅ 极端情况测试完成")

    # 退出码：全部通过=0，有失败=1（退出码供 AI 判断是否需修复，
    # 门禁只看日志中的 ✅ 结束标记）
    import sys
