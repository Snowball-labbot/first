...
def load_with_encoding_detection(file_path: str) -> Tuple[pd.DataFrame, str, float]:
    ...
    # 尝试 chardet 检测（detect_encoding 由 utils 导入；chardet 缺失时
    # utils 侧返回 (None, 0)，自然回退到多编码尝试）
...
def handle_missing_values(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    ...
    for col in df.columns:
        ...
        if missing == 0:
            continue
        # 分母取当前帧长度：循环内删行后仍用初始 total 会高估后续列的
        # 缺失率，导致错误地走"删列"分支。
        total = len(df)
        rate = missing / total
        ...
        if rate < 0.01:
            ...
            # >50% → 真正丢弃该列（避免 NaN 泄漏到下游建模），并在日志与
            # manifest 中记录，供论文 §3/§7 说明理由。
            df = df.drop(columns=[col])
            log.append(f"{col}: 缺失率 {rate:.2%} → ⚠️ 已丢弃该列（论文需说明理由）")
...
def detect_outliers_triangulation(df: pd.DataFrame) -> Dict[str, Dict]:
    ...
    Returns:
        {column_name: {iqr_outliers: [...], zscore_outliers: [...],
                       if_outliers: [...], confirmed: {...}}}
        各 indices 一律为原始 DataFrame 行索引（dropna 前的 label），
        可直接用于定位行；旧版返回 dropna 后的位置索引会错位。
    """
    ...
    for col in numeric_cols:
        # 保留原始行标签：位置 i 的统计结果映射回 orig_index[i]
        series = df[col].dropna()
        data = series.values
        orig_index = np.asarray(series.index)
        if len(data) < 10:
            continue
        ...
        # 方法 3：孤立森林（失败时 if_outliers 为空，不影响两方法投票）
        if_outliers = np.array([], dtype=int)
        try:
        ...
        for idx in range(len(data)):
            ...
            if votes >= 2:
                confirmed.append(int(orig_index[idx]))
        col_results['confirmed'] = {
            'count': len(confirmed),
            'rate': f'{len(confirmed) / len(data):.2%}',
            'indices': confirmed[:20]  # 原始行索引，仅记录前 20 个
        }
...
def run_preprocessing_pipeline(input_path: str, output_dir: str = '结果/') -> Dict:
    ...
    # Step 1: 加载（先记录原始形状与列名——供 manifest 对比预处理前后变化）
    df, encoding, conf = load_with_encoding_detection(input_path)
    original_shape = list(df.shape)
    original_columns = list(df.columns)
    log_lines.append(f"编码: {encoding} (置信度 {conf:.0%}), 原始形状: {df.shape}")
    ...
    manifest = {
        ...
        'original_shape': original_shape,      # 预处理前
        'final_shape': list(df.shape),         # 预处理后（含缺失值处理/丢弃列）
        'dropped_columns': [],                 # 由下方逻辑计算填充
    ...
    # 被丢弃的列：原始列集合 − 当前列集合（若有重名后缀，按前缀匹配）
    final_cols = set(df.columns)
    manifest['dropped_columns'] = sorted(
        {c for c in original_columns if not any(c == fc or fc.startswith(c) for fc in final_cols)})
