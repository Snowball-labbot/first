...
from utils import (save_fig, COLORS, RNG, FIGURES_DIR, OUTPUT_DIR,
                   confidence_interval, bootstrap_ci, redirect_figures)
...
# ⚠️ 展示名 → scipy 分布名 的精确映射。
#    用 getattr(stats, 展示名小写) 查找——'Normal'→'normal'、
#    'Weibull'→'weibull' 在 scipy 中不存在（实际为 norm/weibull_min），
#    'Log-normal' 又直接落回 'norm'，导致"最佳分布叠加曲线"画错或画不出。
DIST_FIT_NAMES = {
    'Normal': 'norm', 'Log-normal': 'lognorm', 'Exponential': 'expon',
    'Gamma': 'gamma', 'Beta': 'beta', 'Weibull': 'weibull_min',
}
...
def plot_univariate_numeric(df, col, dist_results):
    ...
    if dist_results:
        best = dist_results[0]
        try:
            # ⚠️ 用 DIST_FIT_NAMES 精确映射展示名 → scipy 分布名
            dist = getattr(stats, DIST_FIT_NAMES.get(
...
def _safe_filename(name):
    """列名 → 合法文件名（防列名含斜杠/冒号/星号等非法字符导致保存崩溃）"""
    import re as _re
    return _re.sub(r'[\\/*?:"<>|\s]+', '_', str(name)).strip('_') or 'col'
...
def univariate_analysis(df, max_cols=20):
    ...
    for col in numeric_cols:
        ...
        if fig:
            # ⚠️ 列名清洗后再拼文件名（含 '/' 等字符会创建
            #    子目录路径导致保存崩溃）
            save_fig(fig, f'figD1_{_safe_filename(col)}_dist.png')
    return results
...
def bivariate_analysis(df, max_cols=15):
    ...
    if len(numeric_cols) < 2:
        # 与正常分支保持同一返回类型（dict），调用方无需按类型分支
        return {'nonlinear_signals': [], 'n_pairs_strong_corr': 0}
...
def multivariate_analysis(df, max_cols=15):
    ...
    # ⚠️ 标签改用 annotate + offset points 偏移并交替方向——
    #    ax.text 直接放在箭头端点时，多个特征标签在数据密集区互相
    #    重叠（实测 figD3 报 '湿度' vs '风速' 文字重叠），违反"0 处重叠"门禁
    for k, i in enumerate(top_idx):
        x_end = loadings[i, 0] * pca_result[:, 0].std() * 3
        y_end = loadings[i, 1] * pca_result[:, 1].std() * 3
        ax.arrow(0, 0, x_end, y_end,
                 color=COLORS[3], alpha=0.7, width=0.02, head_width=0.1)
        dx, dy = (8, 8) if k % 2 == 0 else (8, -10)
        ax.annotate(numeric_cols[i][:15], (x_end, y_end),
    ...
    # (d) PCA 前 3 主成分 3D 散点图（锁9"数据概览 3D"来源之一，
    #     文件名含 3d 标识便于 Glob 统计；双视角：45°+90°）
    if pca_result.shape[1] >= 3:
        fig3d = plt.figure(figsize=(14, 6))
        ax3d_1 = fig3d.add_subplot(1, 2, 1, projection='3d')
        ax3d_1.scatter(pca_result[:, 0], pca_result[:, 1], pca_result[:, 2],
                       c=pca_result[:, 0], cmap='viridis', alpha=0.7, s=20)
        ax3d_1.view_init(elev=30, azim=45)   # 45° 主视图
        ax3d_1.set_xlabel('PC1'); ax3d_1.set_ylabel('PC2'); ax3d_1.set_zlabel('PC3')
        ax3d_1.set_title('(d) PCA 3D 散点（45°主视图）')
        ax3d_2 = fig3d.add_subplot(1, 2, 2, projection='3d')
        ax3d_2.scatter(pca_result[:, 0], pca_result[:, 1], pca_result[:, 2],
                       c=pca_result[:, 0], cmap='viridis', alpha=0.7, s=20)
        ax3d_2.view_init(elev=90, azim=0)    # 90° 俯视图
        ax3d_2.set_xlabel('PC1'); ax3d_2.set_ylabel('PC2'); ax3d_2.set_zlabel('PC3')
        ax3d_2.set_title('(d) PCA 3D 散点（90°俯视图）')
        plt.tight_layout()
        save_fig(fig3d, 'figD5_3d_pca_scatter.png')
...
def network_analysis(df):
    ...
    # ⚠️ pandas 3.0 写时复制(CoW)强制开启：DataFrame.values / .to_numpy()
    #    均返回只读数组——必须 numpy 层 .copy() 后才能原地修改
    #    （否则 np.fill_diagonal 抛 "underlying array is read-only"）
    adj = (corr > threshold).astype(int).to_numpy().copy()
...
def run_eda_pipeline(input_path, output_dir='结果/'):
    ...
    # --output 语义对图片同样生效：全部 EDA 图跟随报告目录落盘，
    # 而不是写死到 utils 默认目录（修复 output 目录恒为空的问题）。
    redirect_figures(os.path.join(output_dir, 'figures'))
