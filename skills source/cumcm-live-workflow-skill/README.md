...
## 参考文件怎么用（templates/ 自带，直接下载）

| 文件 | 使用时机 | 使用方法 |
|---|---|---|
| `templates/格式规范.pdf` | 读题阶段 | 让 AI **读取全文**，提取"摘要页/页码/页数上限/附录要求"等硬性规定，写作时逐条对照 |
| `templates/AI工具使用规定(2026试行).pdf` | 全程 + 提交前 | 让 AI 读取，确保论文有 **AI 工具使用声明**、支撑材料含 **AI 工具使用详情.pdf**，避免违规 |
| `templates/2026数学建模国赛AI自查表.docx` | 终审阶段 | 用 `python-docx` 提取 26 条核查清单，让 AI **逐条核查成品论文**并输出五段式自查报告 |
| `templates/2026数学建模国赛标准论文Word模板.doc` | 排版阶段（Word 队） | 直接打开按模板填内容（三线表/页码/样式已设好）；LaTeX 队对照其章节结构 |
| `templates/2026国赛国奖级论文AI自动优化升级提示词.docx` | 生成/终审 | 把全文**作为系统提示词喂给 AI**（或让 AI 读完后按九大模块 + 🔴🟡🟢 硬性指标核查/升级论文） |

> 通用流程：把文件放进项目目录 → 会话开始时告诉 AI 路径（如"请读取 `templates/格式规范.pdf` 并对照执行"）→ AI 按 SKILL.md §8 的配套文件说明执行。
...
- **v5.1（2026-08-25）实战排版细节修订**：
  - 列表编号罗马 (i)(ii) → 阿拉伯 (1)(2)（经咨询确认与公式号不混淆）
  - 图表题注编号后不加冒号（labelsep=space）；公式编号完整性（正文每个展示公式都编号）
  - 全篇表格统一 `tabularx{\textwidth}` 撑满版心、禁用 resizebox 压缩、宽表拆分分表
  - 验证板块数据改"文内联动"表述；lstlisting 换 Consolas 覆盖希腊/数学符号
