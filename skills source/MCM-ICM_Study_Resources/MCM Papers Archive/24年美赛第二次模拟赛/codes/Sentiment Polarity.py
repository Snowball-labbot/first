...
# 检查Word列的情感分数并输出
afinn = Afinn()
data['Score'] = data['Word'].apply(lambda x: afinn.score(x))
print(data['Score'])

# 对于这七列，我希望从Percent in 1 try - 6 tries 这七列分别加权系数为1-6，然后加权求和，再排名
Weight = [1, 2, 3, 4, 5, 6]
data['Weighted Percent'] = (data['Percent in 1 try'] * Weight[0] + data['2 tries'] * Weight[1] + data['3 tries'] * Weight[2] + data['4 tries'] * Weight[3] + data['5 tries'] * Weight[4] + data['6 tries'] * Weight[5]) / 100
...
plt.ylim(1, 6)   # 设置y轴范围
plt.scatter(data['Score'], data['Weighted Percent'], color='lightseagreen', alpha=0.5)
plt.show()