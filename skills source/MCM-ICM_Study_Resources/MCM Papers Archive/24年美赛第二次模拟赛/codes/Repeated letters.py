...
# 检查Word列的字母重复数并输出
data['Repeated letters'] = data['Word'].apply(lambda x: Counter(x).most_common(1)[0][1])
print(data['Repeated letters'])
...
plt.xlabel('Repeated letters', fontsize = 18)
plt.ylabel('Score', fontsize = 18)
plt.xticks(range(0, 6), fontsize=14)
plt.yticks(fontsize=14)
plt.xlim(0, 5)  # 设置x轴范围
plt.ylim(1, 6)   # 设置y轴范围
plt.scatter(data['Repeated letters'], data['Score'], color='lightseagreen', alpha=0.5)
plt.show()