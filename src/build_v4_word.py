...
def main():
    temp=ROOT/'.qa/v4_paper';temp.mkdir(parents=True,exist_ok=True)
    source=ROOT/'reports/完整论文_V4.md'
    ...
    for i,p in enumerate(doc.paragraphs):
        ...
        if i==0:
            ...
            if t in ['一 问题重述','二 问题分析','四 符号说明','参考文献'] or t.startswith('附录 '):fmt.page_break_before=True
    ...
    doc.core_properties.title='基于风险校准与价格信息价值的微网购电调度'
    ...
    assert len(old_math)==len(new_math)==45
    for old_node,new_node in zip(old_math,new_math):
        new_node.getparent().replace(new_node,deepcopy(old_node))
    final=ROOT/'reports/完整论文_V4.docx';doc.save(final)
    expected=json.loads((ROOT/'artifacts/v4/paper_manifest.json').read_text(encoding='utf-8'))
    assert len(doc.tables)==expected['tables'] and len(doc.inline_shapes)==expected['figures'] and eqnum==expected['display_equations']
    files=[source,Path(__file__),ROOT/'reports/完整论文_V3.docx',*[ROOT/f for f in code_files],ROOT/'artifacts/v3/run_manifest.json',ROOT/'artifacts/v3/summary.json',ROOT/'artifacts/v3/delivery_selection.json',*[(source.parent/m).resolve() for m in re.findall(r'!\[[^\]]*\]\(([^)]+)\)',source.read_text(encoding='utf-8'))]]
    ...
    (ROOT/'artifacts/v4/word_build.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding='utf-8')
