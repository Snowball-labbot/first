...
def main():
    ...
    for ti,table in enumerate(doc.tables,1):
        ...
        elif columns==3:widths=[6.2,4.8,4.8]
        ...
        for ri,row in enumerate(table.rows):
            ...
            for ci,cell in enumerate(row.cells):
                ...
                for p in cell.paragraphs:
                    ...
                    p.paragraph_format.keep_with_next=ri<len(table.rows)-1 and len(table.rows)<=13
                    p.alignment=WD_ALIGN_PARAGRAPH.CENTER
    ...
    doc.core_properties.title='基于库存价值与滚动决策的微网购电调控'
    ...
    # Pandoc emits matrix properties out of schema order in some versions.
    for node in doc._element.findall('.//'+qn('m:mcPr')):
        node[:]=sorted(node,key=lambda child:0 if child.tag==qn('m:count') else 1)
    for node in doc._element.findall('.//'+qn('m:rPr')):
        if node.find(qn('m:nor')) is not None:
            for child in list(node):
                if child.tag in (qn('m:sty'),qn('m:scr')):node.remove(child)
    ...
    files=[source,Path(__file__),ROOT/'reports/完整论文_V3.docx',*[ROOT/f for f in code_files],ROOT/'artifacts/v3/run_manifest.json',ROOT/'artifacts/v3/summary.json',ROOT/'artifacts/v7/delivery_selection.json',ROOT/'artifacts/v5b/improvement.json',ROOT/'artifacts/v7/model_audit.json',*[(source.parent/m).resolve() for m in re.findall(r'!\[[^\]]*\]\(([^)]+)\)',source.read_text(encoding='utf-8'))]]
