...
def main():
    temp=ROOT/'.qa/v7_paper';temp.mkdir(parents=True,exist_ok=True)
    source=ROOT/'reports/完整论文_V7.md'
    ...
    for i,p in enumerate(doc.paragraphs):
        ...
        if i==0:
            ...
            if t in ['一 问题重述','十一 参考文献'] or t.startswith('附录 '):fmt.page_break_before=True
            ...
            fmt.first_line_indent=Pt(0);fmt.space_before=Pt(7);fmt.space_after=Pt(7)
            fmt.keep_with_next=True
        elif p._element.findall('.//'+qn('w:drawing')):
            fmt.first_line_indent=Pt(0);fmt.space_before=Pt(7);fmt.space_after=Pt(0);fmt.keep_with_next=True
        ...
        if t.startswith('（a）'):fmt.keep_together=True
        ...
        for ri,row in enumerate(table.rows):
            ...
            for ci,cell in enumerate(row.cells):
                ...
                for p in cell.paragraphs:
                    ...
                    p.paragraph_format.keep_with_next=ri<len(table.rows)-1 and (len(table.rows)<=10 or ti==2)
                    p.alignment=WD_ALIGN_PARAGRAPH.CENTER
    ...
    final=ROOT/'reports/完整论文_V7.docx';doc.save(final)
    expected=json.loads((ROOT/'artifacts/v7/paper_manifest.json').read_text(encoding='utf-8'))
    ...
    files=[source,Path(__file__),ROOT/'reports/完整论文_V3.docx',*[ROOT/f for f in code_files],ROOT/'artifacts/v3/run_manifest.json',ROOT/'artifacts/v3/summary.json',ROOT/'artifacts/v5b/delivery_selection.json',ROOT/'artifacts/v5b/improvement.json',ROOT/'artifacts/v5b/verification.json',*[(source.parent/m).resolve() for m in re.findall(r'!\[[^\]]*\]\(([^)]+)\)',source.read_text(encoding='utf-8'))]]
    ...
    (ROOT/'artifacts/v7/word_build.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding='utf-8')
