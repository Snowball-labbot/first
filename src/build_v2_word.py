...
def main():
    temp=ROOT/'.qa/v2_paper';temp.mkdir(parents=True,exist_ok=True)
    source=ROOT/'reports/完整论文_V2.md'
    ...
    code_files=['src/v2_model.py','src/v2_experiments.py','src/v2_verify.py','src/v2_delivery.py','src/q12.py','src/q34_data.py','src/q4_forecast.py']
    ...
    final=ROOT/'reports/完整论文_V2.docx';doc.save(final)
    expected=json.loads((ROOT/'artifacts/v2/paper_manifest.json').read_text(encoding='utf-8'))
    assert len(doc.tables)==expected['tables'] and len(doc.inline_shapes)==expected['figures'] and eqnum==expected['display_equations']
    files=[source,Path(__file__),*[ROOT/f for f in code_files],ROOT/'artifacts/v2/run_manifest.json',ROOT/'artifacts/v2/summary.json',ROOT/'artifacts/v2/delivery_selection.json',*sorted((ROOT/'figures/v2').glob('*.png'))]
    ...
    (ROOT/'artifacts/v2/word_build.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding='utf-8')
