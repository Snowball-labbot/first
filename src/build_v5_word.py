...
def main():
    temp=ROOT/'.qa/v5_paper';temp.mkdir(parents=True,exist_ok=True)
    ...
    code_files=['src/q12.py','src/q34_data.py','src/q4_forecast.py','src/v3_model.py','src/v3_q1.py','src/v3_experiments.py','src/v3_verify.py','src/deliver_q12.py','src/v3_payload.py','scripts/export_v3_xlsx.mjs','src/v5_model.py','src/v5_experiments.py','src/v5_online.py','src/v5_audit.py','src/v5_summary.py','src/v5_delivery.py','src/v5_price_audit.py','src/v5_retained_audit.py']
    ...
    doc.core_properties.title='基于风险校准与在线模型选择的微网购电调度'
    ...
    expected=json.loads((ROOT/'artifacts/v5/paper_manifest.json').read_text(encoding='utf-8'))
    assert len(doc.tables)==expected['tables'] and len(doc.inline_shapes)==expected['figures'] and eqnum==expected['display_equations']
    files=[source,Path(__file__),ROOT/'reports/完整论文_V3.docx',*[ROOT/f for f in code_files],ROOT/'artifacts/v3/run_manifest.json',ROOT/'artifacts/v3/summary.json',ROOT/'artifacts/v5/delivery_selection.json',ROOT/'artifacts/v5/improvement.json',ROOT/'artifacts/v5/verification.json',*[(source.parent/m).resolve() for m in re.findall(r'!\[[^\]]*\]\(([^)]+)\)',source.read_text(encoding='utf-8'))]]
    ...
    (ROOT/'artifacts/v5/word_build.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding='utf-8')
