...
def main():
    temp=ROOT/'.qa/v5b_paper';temp.mkdir(parents=True,exist_ok=True)
    source=ROOT/'reports/完整论文_V5.md'
    ...
    for i,p in enumerate(doc.paragraphs):
        ...
        if i==0:
            ...
            if t in ['一 问题重述','二 问题分析','参考文献'] or t.startswith('附录 '):fmt.page_break_before=True
    ...
    code_files=['src/q12.py','src/q34_data.py','src/q4_forecast.py','src/v3_model.py','src/v3_q1.py','src/v3_experiments.py','src/v3_verify.py','src/deliver_q12.py','src/v3_payload.py','scripts/export_v3_xlsx.mjs','src/v5_model.py','src/v5_experiments.py','src/v5_online.py','src/v5_audit.py','src/v5_summary.py','src/v5_delivery.py','src/v5_price_audit.py','src/v5_retained_audit.py','src/v5b_value.py','src/v5b_experiments.py','src/v5b_rolling.py','src/v5b_windows.py','src/v5b_analysis.py','src/v5b_delivery.py']
    ...
    doc.core_properties.title='基于风险校准与库存价值控制的微网购电调度'
    ...
    # Preserve V3 OMML verbatim: converter versions can move primes relative to subscripts.
    prior=Document(ROOT/'reports/完整论文_V3.docx')
    old_math=prior._element.findall('.//'+qn('m:oMath'))
    new_math=doc._element.findall('.//'+qn('m:oMath'))
    # Match unchanged equations to a conversion of V4; retain new V5 equations.
    old_converted=temp/'prior_math.docx'
    markdown_to_docx(ROOT/'reports/完整论文_V4.md',old_converted,reference,overwrite=True)
    def signature(node):return ''.join(x.text or '' for x in node.findall('.//'+qn('m:t')))
    old_signatures=[signature(x) for x in Document(old_converted)._element.findall('.//'+qn('m:oMath'))]
    cursor=0
    for new_node in new_math:
        if cursor<len(old_math) and signature(new_node)==old_signatures[cursor]:
            new_node.getparent().replace(new_node,deepcopy(old_math[cursor]));cursor+=1
    assert cursor==len(old_math)==45,(cursor,len(old_math))
    final=ROOT/'reports/完整论文_V5.docx';doc.save(final)
    expected=json.loads((ROOT/'artifacts/v5b/paper_manifest.json').read_text(encoding='utf-8'))
    assert len(doc.tables)==expected['tables'] and len(doc.inline_shapes)==expected['figures'] and eqnum==expected['display_equations']
    ...
    (ROOT/'artifacts/v5b/word_build.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding='utf-8')
