...
def main():
    ...
    for key,label in [('3','q3_online'),('4-3','q4_online')]:
        ...
        note=wb['口径说明'];note['B2']='V5：每月以此前28天共同初始SOC验证，选择基础或自适应负载预测；实际SOC连续跨月。'
    ...
    dump_json(OUT/'delivery_selection.json',{'sources':SOURCES,'main':'Q3 and Q4 rolling use causal monthly selection; Q1/Q2/Q4 ahead retained','copied_byte_identical':['1','2','4-2']})
    dump_json(OUT/'xlsx_audit.json',dict(status='PASS',updated=audit,copied={k:hashlib.sha256((OUT/f'result{k}.xlsx').read_bytes()).hexdigest() for k in ['1','2','4-2']}));print(audit)
