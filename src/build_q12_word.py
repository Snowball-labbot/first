...
def main():
    temp=ROOT/'.qa/word_revision';temp.mkdir(parents=True,exist_ok=True)
    source=ROOT/'reports/Q1_Q2修订稿.md'
    ...
    for name in ['Normal','Body Text','First Paragraph']:
        if name in doc.styles:
            st=doc.styles[name];font(st);st.paragraph_format.line_spacing=1.2
    ...
    for name,size,bold,east in [('Normal',12,False,'宋体'),('Body Text',12,False,'宋体'),
        ('First Paragraph',12,False,'宋体'),('Title',16,True,'黑体'),('Subtitle',10.5,False,'宋体'),
    ...
    for i,p in enumerate(doc.paragraphs):
        ...
        fmt.space_before=Pt(0);fmt.space_after=Pt(5);fmt.line_spacing=1.2
        ...
        if i==0:
            ...
            if t in ['一 问题重述','四 符号说明','参考文献']:fmt.page_break_before=True
    ...
    doc.core_properties.title='考虑预测误差的微网日前购电与储能调度'
    ...
    final=ROOT/'reports/Q1_Q2修订初稿.docx';doc.save(final)
    expected=json.loads((ROOT/'artifacts/q12_revision/report_manifest.json').read_text(encoding='utf-8'))
    assert len(doc.tables)==16 and len(doc.inline_shapes)==8 and eqnum==len(expected['display_equations'])
    files=[source,ROOT/'reports/Q1_Q2公式.tex',Path(__file__),*sorted((ROOT/'figures/q12_revision').glob('*.png')),
    ...
    manifest={'docx':str(final.relative_to(ROOT)),'sha256':sha(final),'display_equations':eqnum,
        ...
        'native_math_objects':len(doc._element.findall('.//'+qn('m:oMath'))),'figures':8,'tables':16,
        ...
        'scope':'First two questions only; not a complete competition submission'}
    (ROOT/'artifacts/q12_revision/word_build.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding='utf-8')
