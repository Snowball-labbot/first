...
import os
import shutil
os.environ["PATH"]=r"D:/anaconda/Library/bin"+os.pathsep+os.environ.get("PATH","")
...
def main():
    temp=ROOT/'.qa/v8_paper';temp.mkdir(parents=True,exist_ok=True)
    source=ROOT/'reports/完整论文_V8.md'
    reference=temp/'reference.docx'
    shutil.copyfile(ROOT/'reports/完整论文_V6.docx',reference)
    ...
    for name,size,bold,east in [('Normal',11.5,False,'宋体'),('Body Text',11.5,False,'宋体'),
        ...
        if name in doc.styles:
            font(doc.styles[name],size,bold,east)
            doc.styles[name].paragraph_format.page_break_before=False
    ...
    numbered_captions=set(re.findall(r'!\[[^\]]+\]\([^)]+\)\s*\n\s*(图[^\n]+)',manuscript))
    numbered_captions.update(re.findall(r'^(表[^\n]+)\n+(?=\|)',manuscript,re.M))
    ...
    for i,p in enumerate(doc.paragraphs):
        ...
        if t=='一 问题重述':in_abstract=False
        fmt.space_before=Pt(0);fmt.space_after=Pt(3);fmt.line_spacing=1.12
        fmt.page_break_before=False;fmt.keep_with_next=False;fmt.keep_together=False
        ...
        if in_abstract and i>0 and not p.style.name.startswith('Heading'):
            fmt.line_spacing=1.30;fmt.space_after=Pt(7)
        ...
        if i==0:
            ...
            fmt.first_line_indent=Pt(0);fmt.space_before=Pt(8);fmt.space_after=Pt(5)
            fmt.keep_with_next=True
            p.alignment=WD_ALIGN_PARAGRAPH.CENTER if p.style.name=='Heading 1' else WD_ALIGN_PARAGRAPH.LEFT
            if t=='一 问题重述' or t.startswith('附录 '):fmt.page_break_before=True
        elif t.startswith('关键词：'):
            fmt.first_line_indent=Pt(0);fmt.space_before=Pt(6)
        elif t in numbered_captions:
            p.style=doc.styles['Caption'];p.alignment=WD_ALIGN_PARAGRAPH.CENTER
            fmt.first_line_indent=Pt(0);fmt.space_before=Pt(4);fmt.space_after=Pt(5)
            fmt.keep_with_next=t.startswith('表')
        elif p._element.findall('.//'+qn('w:drawing')):
            fmt.first_line_indent=Pt(0);fmt.space_before=Pt(5);fmt.space_after=Pt(0);fmt.keep_with_next=True
    ...
    for ti,table in enumerate(doc.tables,1):
        ...
        elif ti==10:widths=[5.8,5,5]
        elif ti==11:widths=[2.1,3.5,6.6,3.6]
        ...
        for ri,row in enumerate(table.rows):
            ...
            for ci,cell in enumerate(row.cells):
                ...
                for p in cell.paragraphs:
                    ...
                    p.paragraph_format.keep_with_next=ri==0 or (ri<len(table.rows)-1 and len(table.rows)<=9)
                    p.alignment=WD_ALIGN_PARAGRAPH.CENTER
    ...
    code_files=['src/q12.py', 'src/q34_data.py', 'src/v3_model.py', 'src/v5_model.py', 'src/v5b_value.py', 'src/v7_price.py', 'src/v7_dispatch.py']
    ...
    footer=sec.footer.paragraphs[0];footer.clear();footer.alignment=WD_ALIGN_PARAGRAPH.CENTER
    fld=OxmlElement('w:fldSimple');fld.set(qn('w:instr'),'PAGE')
    run=OxmlElement('w:r');text=OxmlElement('w:t');text.text='1';run.append(text);fld.append(run);footer._element.append(fld)
    doc.core_properties.title='考虑供需与电价不确定性的微网购电调度'
    ...
    # Reuse original OMML where its mathematical text matches the new conversion.
    prior=Document(ROOT/'reports/完整论文_V6.docx')
    def signature(node):return ''.join(x.text or '' for x in node.findall('.//'+qn('m:t')))
    old_math={signature(x):x for x in prior._element.findall('.//'+qn('m:oMath'))}
    retained=0
    for node in list(doc._element.findall('.//'+qn('m:oMath'))):
        key=signature(node)
        if key in old_math:
            node.getparent().replace(node,deepcopy(old_math[key]));retained+=1
    final=ROOT/'reports/完整论文_V8.docx';doc.save(final)
    expected=json.loads((ROOT/'artifacts/v8/paper_manifest.json').read_text(encoding='utf-8'))
    assert len(doc.tables)==expected['tables'] and len(doc.inline_shapes)==expected['figures'] and eqnum==expected['display_equations'], (len(doc.tables),len(doc.inline_shapes),eqnum,expected)
    files=[source,Path(__file__),ROOT/'reports/完整论文_V6.docx',ROOT/'src/v8_prepare.py',ROOT/'src/v8_report.py',ROOT/'artifacts/v8/q1_summary.json',ROOT/'artifacts/v8/q1_intervals.csv',ROOT/'scripts/v8_figures.m',*[ROOT/f for f in code_files],ROOT/'artifacts/v3/run_manifest.json',ROOT/'artifacts/v3/summary.json',ROOT/'artifacts/v5b/delivery_selection.json',ROOT/'artifacts/v5b/improvement.json',ROOT/'artifacts/v5b/verification.json',*[(source.parent/m).resolve() for m in re.findall(r'!\[[^\]]*\]\(([^)]+)\)',source.read_text(encoding='utf-8'))]]
    manifest={'docx':str(final.relative_to(ROOT)),'sha256':sha(final),'display_equations':eqnum,
        ...
        'scope':'Four-question revised manuscript with core program appendix', 'retained_math_by_signature':retained, 'template_source':'reports/完整论文_V6.docx'}
    (ROOT/'artifacts/v8/word_build.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding='utf-8')
