...
import sys
import os
os.environ["PATH"]='D:/anaconda/Library/bin'+os.pathsep+os.environ.get("PATH","")
...
def main():
    temp=ROOT/'.qa/v3_paper';temp.mkdir(parents=True,exist_ok=True)
    source=ROOT/'reports/完整论文_V3.md'
    ...
    final=ROOT/'reports/完整论文_V3.docx';doc.save(final)
    expected=json.loads((ROOT/'artifacts/v3/paper_manifest.json').read_text(encoding='utf-8'))
    assert len(doc.tables)==expected['tables'] and len(doc.inline_shapes)==expected['figures'] and eqnum==expected['display_equations']
    files=[source,Path(__file__),*[ROOT/f for f in code_files],ROOT/'artifacts/v3/run_manifest.json',ROOT/'artifacts/v3/summary.json',ROOT/'artifacts/v3/delivery_selection.json',*sorted((ROOT/'figures/v3').glob('*.png'))]
    ...
    (ROOT/'artifacts/v3/word_build.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding='utf-8')
