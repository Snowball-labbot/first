...
out=root/'.qa/v12/final';out.mkdir(parents=True,exist_ok=True)
...
try:
 print('Opening V12',flush=True);doc=app.Documents.Open(str(root/'reports/完整论文_V12.docx'),ReadOnly=True,AddToRecentFiles=False)
 doc.Repaginate()
 pairs=[]
 ...
 (root/'artifacts/v12/figure_page_pairs.json').write_text(json.dumps(pairs,ensure_ascii=False,indent=2),encoding='utf8')
 print('Exporting PDF',flush=True);doc.ExportAsFixedFormat(str(root/'reports/完整论文_V12.pdf'),17)
finally:
 if doc is not None:doc.Close(False)
 app.Quit()
p=fitz.open(root/'reports/完整论文_V12.pdf');paths=[];stats=[]
...
sheets(paths,out);(root/'artifacts/v12/page_audit.json').write_text(json.dumps(stats,ensure_ascii=False,indent=2),encoding='utf8')
